...
public class Statistics {
    ...
    public void saveData(int daysOfSimulation, int totalEncounter, int sendHome, boolean withRandom, List<Integer> variance) {
        try (FileWriter fw = new FileWriter("res/statistics.txt", true);
            ...
            double vari = calculateVariance(variance, monthlyAvg, daysOfSimulation);
            out.println("the variance is: " + vari);
            out.println("the standard deviation is: " + Math.sqrt(vari));
        } catch (IOException e) {
            System.out.println(e);
        }
    }

    private double calculateVariance(List<Integer> var, double mAvg, int totalDays){
        double dummy = 0;
        for(int i: var){
            dummy += ((double)i - mAvg) * ((double)i - mAvg);
        }
        encounterVariance = dummy / (double) totalDays;
